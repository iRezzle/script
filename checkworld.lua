...
webhookeli("@everyone `SCRIPT CHECK FARM MADE By : `<@315051522918580224>")
for i = 1, #farmList do
    ...
    if getBot().world == farmList[i]:upper() and not NUKED then
        webhookeli("`"..farmList[i]:upper().." | "..scanTree(tree).." READY | "..scanUnready(tree).." UN-READY | "..scanFossil().." FOSSIL`")
        sleep(1000)
        hah = 0
    elseif getBot().world ~= farmList[i]:upper() and NUKED then
        webhookeli("`"..farmList[i]:upper().." | NUKED`")
        sleep(1000)
        NUKED = false
        hah = 0
        foundNukedFarm = true
...
webhookeli("`TOTAL NUKED : "..total.." WORLD`")
